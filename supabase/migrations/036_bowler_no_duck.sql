-- Bowlers are exempt from the duck penalty (-2 pts when out for 0).
-- They aren't expected to score with the bat, so the penalty unfairly
-- punishes specialist bowlers.
...
BEGIN
  ...
  -- Duck penalty — bowlers exempt
  IF p_is_out AND p_runs = 0 AND p_role <> 'bowler' THEN
    pts := pts - 2;
  END IF;
  ...
  IF p_role IN ('batsman','wicket_keeper','all_rounder')
     AND p_balls_faced > 0
     AND (p_balls_faced >= 10 OR p_runs >= 20) THEN
